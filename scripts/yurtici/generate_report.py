#!/usr/bin/env python3
"""
Yurtiçi Kargo CrowdStrike Weekly Health Check Report
...
def main():
    ...
    print(f"[{date_str}] Yurtiçi Kargo raporu oluşturuluyor...")
    ...
    # ── GRUPLARI ÇEK ───────────────────────────────────────
    groups_r = requests.get(f"{BASE_URL}/devices/combined/host-groups/v1", headers=H, timeout=10)
    ...
    for ptype, ep in [("prevention", "/policy/combined/prevention/v1"),
        ...
        r = requests.get(f"{BASE_URL}{ep}?limit=100", headers=H, timeout=10)
        if r.status_code == 200:
    ...
    for g in group_list:
        ...
        for ptype, ep in POLICY_TYPE_MAP.items():
            r = requests.get(f"{BASE_URL}{ep}", headers=H,
                params={"filter": f"groups:['{gid}']"}, timeout=10)
            if r.status_code == 200:
    ...
    for i, (label, value) in enumerate([
        ...
        ("Prepared by", "Pure7"), ("Customer", "Yurtiçi Kargo"),
    ...
    ws_sum["A1"] = "Yurtiçi Kargo – CrowdStrike Security Dashboard"
    ...
    ws_risk["A1"] = "Yurtiçi Kargo – Risk & Action Required"
    ...
    ws_hosts["A1"] = f"Yurtiçi Kargo – CrowdStrike Host Inventory  ({len(hosts)} hosts)"
    ...
    ws1["A1"] = "Yurtiçi Kargo – CrowdStrike Host Grupları"
    ...
    ws2["A1"] = "Yurtiçi Kargo – Host Grubu / Policy Atamaları"
    ...
    ws3["A1"] = "Yurtiçi Kargo – CrowdStrike Policy Details"
    ...
    ws_excl["A1"] = f"Yurtiçi Kargo – CrowdStrike Exclusions  ({len(ml_details)+len(ioa_details)+len(sv_details)} total)"
    ...
    ws_hm["A1"] = f"Yurtiçi Kargo – High Memory Usage (>80%)  ({len(hm_hosts)} hosts)"
    ...
    # ── KAYDET ──────────────────────────────────────────────
    filename = f"Yurtici_Kargo_CrowdStrike_HealthCheck_{date_str}.xlsx"
